...
def _brief(u: User) -> dict:
    return {
        "id": u.id, "name": u.name, "account": u.account, "dept": u.dept,
...
async def overview(
    ...
    return ok({
        "departments": len({u.dept.split(" / ")[0] for u in (await session.execute(select(User))).scalars()}),
...
async def create_user(
    ...
    new_user = User(
        id=gen_id("u"), name=body.name, account=body.account,
