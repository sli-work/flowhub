...
router = APIRouter(prefix="/api/v1/agents/access-keys", tags=["agents"])
...
async def create_key(
    ...
    await AuditService(session).record(
        actor=user.name, action="agent:access_key_create", target=key.name, result="success",
    )
...
async def revoke_key(
    ...
    key.status = "revoked"
    await AuditService(session).record(
        actor=user.name, action="agent:access_key_revoke", target=key.name, result="success",
    )
    await session.commit()
