...
function FieldControl({ field, value, onChange, error, workItemId, project }: {
  ...
  switch (field.type) {
    ...
    case 'file': {
      ...
      return <UploadWidget value={arr} multiple={multiple} onChange={onChange} error={error} workItemId={workItemId} project={project} />
    }
    default:
      return (
        <input
...
function UploadWidget({ value, multiple, onChange, error, workItemId, project }: {
  value: UploadValue[]
  multiple: boolean
  onChange: (v: unknown) => void
  error?: boolean
        ...
        <div className="mt-1.5 flex flex-wrap gap-1.5">
          {value.map((item) => {
            ...
            return <span key={ref.id} className="inline-flex items-center gap-1 rounded-full bg-slate-100 px-2 py-0.5 text-[11px] font-medium text-slate-600 dark:bg-slate-800 dark:text-slate-300">
              {ref.name}
              <button onClick={() => onChange(value.filter((entry) => (typeof entry === 'string' ? entry : entry.id) !== ref.id))}
...
export function SchemaForm({ fields, values, onChange, compact, workItemId, project }: {
  fields: FormField[]
  values: SchemaValues
      ...
      {fields.map((f) => {
        ...
        return (
          <div key={f.key} className={compact ? '' : 'space-y-1.5'}>
            ...
            <FieldControl field={f} value={values[f.key]} onChange={(v) => set(f.key, v)} error={error} workItemId={workItemId} project={project} />
