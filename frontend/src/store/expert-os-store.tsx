...
interface ExpertOsContextValue {
  ...
  addProvider: (input: { id?: string; name: string; baseUrl: string; models: string[]; apiKey: string; maxContextTokens?: number; modelLimits?: Record<string, { max_context_tokens: number | null; max_output_tokens: number | null }>; visionModels?: string[] }) => Promise<void>
...
export function ExpertOsProvider({ children }: { children: ReactNode }) {
  ...
  addProvider: async (input) => {
    if (input.id) await api.patch(`/api/v1/providers/${input.id}`, { name: input.name, base_url: input.baseUrl, api_key: input.apiKey, models: input.models, max_context_tokens: input.maxContextTokens ?? 32_000, model_limits: input.modelLimits ?? {}, vision_models: input.visionModels ?? [] })
    else await api.post('/api/v1/providers', { name: input.name, base_url: input.baseUrl, api_key: input.apiKey, models: input.models, max_context_tokens: input.maxContextTokens ?? 32_000, model_limits: input.modelLimits ?? {}, vision_models: input.visionModels ?? [] })
