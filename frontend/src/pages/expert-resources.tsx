...
export function ResourceCenterPage({ kind }: { kind: ResourceKind }) {
  ...
  return <div className="page-container">
    ...
    {kind === 'mcp' && <ResourceTable rows={rows as McpServerRecord[]} onRowClick={setSelected} columns={[{ key: 'name', label: 'MCP Server', render: (item) => <ResourceName icon={<Cable className="h-4 w-4" />} name={item.name} sub={`${item.direction} · ${item.transport}`} /> }, { key: 'endpoint', label: 'Endpoint', render: (item) => item.endpoint || '内置' }, { key: 'tools', label: 'Tools', render: (item) => Array.isArray(item.tools) ? item.tools.length : item.tools }, { key: 'status', label: '状态', render: (item) => <OsStatusBadge status={item.status} /> }, { key: 'action', label: '操作', render: (item) => <div className="flex items-center gap-1"><button type="button" className="rounded-md px-2 py-1 text-[11px] text-blue-600 hover:bg-blue-50" onClick={(event) => { event.stopPropagation(); setEditingMcp(item); setMcpOpen(true) }}>编辑</button><button type="button" aria-label={`删除 ${item.name}`} className="rounded-md p-1.5 text-slate-400 hover:text-red-600" onClick={(event) => { event.stopPropagation(); if (window.confirm(`确认删除 MCP Server「${item.name}」？`)) void deleteMcpServer(item.id).then(() => toast.success('MCP Server 已删除')).catch((error: unknown) => toast.error(error instanceof Error ? error.message : '删除失败')) }}><Trash2 className="h-4 w-4" /></button></div> }]} />}
    ...
    <DetailDrawer open={!!selected && kind !== 'provider'} title={selected && 'name' in selected ? selected.name : ''} eyebrow={meta.title} onClose={() => setSelected(null)}>{selected && <div className="space-y-3 text-sm"><p className="text-slate-500">{'description' in selected ? selected.description : '资源详情'}</p>{kind === 'skill' && <div className="grid grid-cols-2 gap-2 text-xs"><span>文件：{(selected as SkillRecord).filename ?? '—'}</span><span>类型：{(selected as SkillRecord).packageType ?? '—'}</span><button className="col-span-2 mt-2 rounded-lg border border-red-200 px-3 py-2 text-red-600 hover:bg-red-50" onClick={() => void removeSkill(selected as SkillRecord)}>删除 Skill</button></div>}{kind === 'mcp' && <><button className="rounded-lg border border-blue-200 px-3 py-2 text-xs text-blue-600 hover:bg-blue-50" onClick={() => { setEditingMcp(selected as McpServerRecord); setMcpOpen(true) }}>编辑 JSON 配置</button><McpToolList server={selected as McpServerRecord} onUpdate={updateMcpTool} /></>}</div>}</DetailDrawer>
...
function ProviderDialog({ open, provider, onClose, onSave }: { open: boolean; provider: ProviderRecord | null; onClose: () => void; onSave: (input: { id?: string; name: string; baseUrl: string; models: string[]; apiKey: string; maxContextTokens?: number }) => Promise<void> }) {
  const [name, setName] = useState(provider?.name ?? '')
  const [baseUrl, setBaseUrl] = useState(provider?.baseUrl ?? '')
  const [models, setModels] = useState(provider?.models.join(', ') ?? '')
  const [apiKey, setApiKey] = useState('')
  const [maxContextTokens, setMaxContextTokens] = useState(String(provider?.maxContextTokens ?? 1_000_000))
  const [busy, setBusy] = useState(false)
  const [tested, setTested] = useState(false)
  const [testResult, setTestResult] = useState('')
  useEffect(() => { setName(provider?.name ?? ''); setBaseUrl(provider?.baseUrl ?? ''); setModels(provider?.models.join(', ') ?? ''); setApiKey(''); setMaxContextTokens(String(provider?.maxContextTokens ?? 1_000_000)); setTested(false); setTestResult('') }, [provider, open])
  ...
  const submit = async () => { if (!tested) return; setBusy(true); try { await onSave({ id: provider?.id, name: name.trim(), baseUrl: baseUrl.trim(), models: models.split(',').map((item) => item.trim()).filter(Boolean), apiKey, maxContextTokens: Number(maxContextTokens) || 1_000_000 }); onClose() } finally { setBusy(false) } }
  return <Dialog open={open} onOpenChange={(next) => !next && onClose()}><DialogContent><DialogHeader><DialogTitle>{provider ? `编辑 Provider · ${provider.name}` : '新增 Provider'}</DialogTitle></DialogHeader><div className="space-y-3"><label className="grid gap-1 text-sm">名称<Input value={name} onChange={(event) => { setName(event.target.value); setTested(false) }} /></label><label className="grid gap-1 text-sm">Base URL<Input value={baseUrl} onChange={(event) => { setBaseUrl(event.target.value); setTested(false) }} /></label><label className="grid gap-1 text-sm">模型列表<Input value={models} onChange={(event) => { setModels(event.target.value); setTested(false) }} /></label><label className="grid gap-1 text-sm">最大上下文窗口<Input value={maxContextTokens} onChange={(event) => setMaxContextTokens(event.target.value)} /></label><label className="grid gap-1 text-sm">API Key<Input type="password" value={apiKey} onChange={(event) => { setApiKey(event.target.value); setTested(false) }} /></label>{testResult && <div className="rounded-lg bg-slate-50 p-2 text-xs">{testResult}</div>}</div><DialogFooter><Button variant="outline" onClick={onClose}>取消</Button><Button variant="outline" disabled={busy} onClick={() => void test()}>{busy ? '检测中…' : '检测连接'}</Button><Button disabled={busy || !tested} onClick={() => void submit()}>{provider ? '保存修改' : '创建 Provider'}</Button></DialogFooter></DialogContent></Dialog>
}
